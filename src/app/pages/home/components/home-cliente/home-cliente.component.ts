...
export class HomeClienteComponent implements OnInit, OnDestroy {
  ...
  get tieneMesaAsignada(): boolean {
    // Se habilita si el pedido existe y el maitre ya le asignó un ID de mesa
    return this.pedidoActual != null && this.pedidoActual.idMesa != null;
  }

  get hizoUnPedido(): boolean {
    // Se habilita si el pedido existe y su estado avanzó más allá de la simple asignación de mesa
    if (!this.pedidoActual) return false;
    const estadosSinPedido = ['esperando mesa', 'mesa asignada', 'rechazado'];
    return !estadosSinPedido.includes(this.pedidoActual.estado);
  }
